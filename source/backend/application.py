...
def parseJSON():
    ...
    imgType = dataDict['type']

    # format and execute query
    query = queryGenerator(name, classType, timePeriod, imgType)
    # return query
    cur.execute(query)
...
def queryGenerator(name, classType, timePeriod, imgType):
    # in progress
    today = datetime.datetime.now().date()
    quer = ""
    if imgType == "T":
      query = "SELECT name, class, upload_time, thumbnail_pth, confidence FROM imageuploads WHERE "
    else:
...
def analyze_json():
    ...
    confidence = abs(scores[0][0] - scores[0][1]) / 4 * 100
    if confidence > 100:
      confidence = 100.0
    ...
    return json.dumps({
        ...
        "certainty": '{0:.2f}'.format(confidence)
    })
